        ...
        for(size_t i = 0; 
           ...
           ++i) {
            ...
            if (tmp->symbol == TOK_TYPEID) {
                string *find = const_cast<string*>(tmp->lexinfo);
                if (struct_table.find(find) == 
                   struct_table.end()) {
/*
...
*/
                    ...
                    node->children[1]->children[i]->struct_name = find;
                    // return true;
                }
            }
            fprintf(outFile, "   ");
            ...
            if (tmp->symbol == TOK_ARRAY) {
                init_symbol(tmp->children[0], field_sym);
                    field_sym->attributes[ATTR_array] = 1;
            }
            else {
                init_symbol(tmp, field_sym);
            }
            ...
            tbl->insert(field);
            write_symbol_to_file(outFile, field_sym, field_name);
            if(tmp->symbol == TOK_ARRAY) {
                  tmp->children[1]->attributes =
                      field_sym->attributes;
                  tmp->children[1]->struct_name = sym->struct_name;
            } else {
                  tmp->children[0]->attributes =
                      field_sym->attributes;
                  tmp->children[0]->struct_name = sym->struct_name;
            }
        }
    }
    pair <string, symbol_table*> str(*name, tbl);
    // printf("handle_struct: 
    // inserting name %s\n", name->c_str());
...
bool create_identifiers (FILE* outFile, astree* node) {
    if (node != nullptr && node->symbol == ROOT)
    {
        symbol_stack.push_back(new symbol_table);
        block_stack.push_back(next_block++);
        valid_file = false;
...
bool type_check (astree* node) {
    ...
    case '.': {
        ...
        if(str->find(field_name) == str->end()) {
            errprintf("%zd:%zd.%zd: error: ", 
                     "%s: no field in struct\n", 
                     node->lloc.filenr, node->lloc.linenr, 
                     node->lloc.offset, *field_name);
            return false;
        }
    ...
    case TOK_NEWARRAY: {
        ...
        if (attrs[ATTR_void] || 
           ...
           attrs[ATTR_void] || 
          (attrs[ATTR_struct] && 
          node->children[0]->struct_name == nullptr)) {
            errprintf("%zd:%zd.%zd: error: ",
